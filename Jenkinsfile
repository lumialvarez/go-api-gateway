	...
	stages {
		...
		stage('Deploy') {
			steps {
				...
				sh "ssh ${SSH_MAIN_SERVER} 'sudo docker run --name go-api-gateway --net=backend-services --add-host=lmalvarez.com:${INTERNAL_IP} -p 9192:9191 -e SCOPE='prod' -d --restart unless-stopped go-api-gateway:latest'"
			}
		}
	}
}